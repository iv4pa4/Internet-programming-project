...
import { FormData } from '@/types/adventure';
...
const FormNewAdventure = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await axios.post('api/adventures', formData);

      if (response.status === 200) {
        router.push(`/adventures/${response.data.newAdventure.id}`);
      }
    } catch (error) {
      console.error(error);
    }
...
export default FormNewAdventure;
